...
2. Los demás elementos usan .innerText para acceder 
y modificar al texto que aparece dentro. 
...
//TAREA 0: completar tareas/clase-5/index.html para que incluya tarea-clase-5.js
...
$calculateButton.onclick = function (e) {
  e.preventDefault();
  //Obtaining the input value --> annual salary
  const annualSalary = Number(document.querySelector("#annual-salary").value);
  const MONTH_OF_YEAR = 12;
  const monthlySalary = annualSalary / MONTH_OF_YEAR; // --> monthly salary

  // Dynamically creating the disabled input
  ////<input type="text" disabled id="salario-mensual"/>
  const salaryBox = document.createElement('input');
  salaryBox.type = "text";
  salaryBox.disabled = true;
  salaryBox.id = "monthly-salary";
  salaryBox.value = monthlySalary;
  
  //Adding the monthly salary to the disabled input 
  const $nodePage = document.querySelector("body");
  $nodePage.appendChild(salaryBox);
